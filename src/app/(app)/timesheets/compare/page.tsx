...
export default function CompareTimesheetsPage() {
  ...
  async function fetchAll() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.error || \"Failed to fetch time entries\");
  ...
  return (
    <div className=\"container mx-auto py-8 space-y-6\">
      <Card className=\"shadow-lg\">
        <CardHeader>
          <CardTitle className=\"text-3xl font-bold\">Comparison Timesheets</CardTitle>
        </CardHeader>
            ...
            {entries.map((e: any) => (
              <div key={e.id} className=\"border rounded-md p-3 space-y-2\">
                <div className=\"flex items-center justify-between\">
                  ...
                  <div className=\"text-sm font-semibold\">{formatHoursHM(e.hours)}</div>
                </div>
                <div className=\"text-sm\">
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Lock className=\"h-4 w-4 text-amber-500 cursor-help\" aria-label=\"Locked\" />
                          </TooltipTrigger>
                          <TooltipContent>
                            <div className=\"max-w-xs text-wrap\">
                              {e.locked_reason || \"This project's hours are locked for this period.\"}
                            </div>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                  ...
                  <div className=\"text-muted-foreground\">{e.task?.name || \"-\"}</div>
                </div>
