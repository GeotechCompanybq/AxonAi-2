...
export default function GrayquarterTimesheetsPage() {
    ...
    try {
      ...
      for (const e of offenders) {
        ...
        try {
          ...
          setEntries((prev) =>
            prev.map((row: any) => (row.id === e.id ? { ...row, ...updated } : row))
          );
        } catch {}
      }
    } finally {
  ...
  async function handleDelete(id: string | number) {
    ...
    try {
      const uid = readFirebaseAuthUid();
      const url = new URL(`/api/harvest/timesheets/${id}`, window.location.origin);
      url.searchParams.set("conn", "alt");
      if (uid) url.searchParams.set("uid", uid);
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-44">
                      ...
                      <DropdownMenuItem
                        ...
                        disabled={deletingId === e.id || isLoading || e.is_locked}
