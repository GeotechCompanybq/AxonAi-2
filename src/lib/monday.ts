export async function fetchMondayTasks(token: string) {
  ...
  async function graphql<T = any>(query: string, variables: Record<string, any>) {
  ...
  const meJson: any = await graphql(meQuery, {});
  ...
  if (!supportsBoardsPage) {
    ...
    for (const b of arr) boards.push({ id: String(b.id), name: String(b.name) });
  }
  ...
  function mapStatus(raw: string | undefined): string {
    ...
    if (s.includes("working") || s.includes("in progress") || s.includes("progress"))
      return "inprogress";
  ...
  for (const it of withDueDate) {
    ...
    results.push({
      name: it.name,
      description: `${it.boardName}${it.group?.title ? " · " + it.group.title : ""}`,
