...
export function CreateScheduleForm() {
  ...
  const [scheduleTextOutput, setScheduleTextOutput] = useState<string | null>(
    null
  );
      ...
      if (result.tasks && result.tasks.length > 0) {
          ...
          if (aiTask.dueDate) {
            try {
              ...
              if (isValid(parsedDate)) {
                ...
                console.warn(
                  `Invalid dueDate format from AI: ${aiTask.dueDate}`
                );
              }
            } catch (e) {
              console.warn(
                `Error parsing dueDate from AI: ${aiTask.dueDate}`,
                e
              );
            }
          }
        ...
        newTasksFromAI.forEach((aiTask) => {
          if (
            !existingTasks.some(
              (existingTask) =>
                existingTask.name.toLowerCase() === aiTask.name.toLowerCase()
            )
          ) {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto shadow-xl">
      <CardHeader>
        ...
        <CardDescription>
          Describe your ideal schedule, and let Axon AI craft it for you. Tasks
          identified by the AI (including priority and due date if applicable)
          will be automatically added to your "My Tasks" page. Try something
          like: "Create a daily routine for a freelance writer working from
          home, who needs to finish a draft by Friday." or "Plan my study
          schedule for upcoming final exams in Math (high priority) and Physics
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel htmlFor="scheduleDescription" className="text-lg">
