...
export function ProgressPieChart() {
  ...
  useEffect(() => {
    ...
    const dataForChart: ChartDataItem[] = [
      {
        name: chartConfig.todo.label as string,
        value: counts.todo,
        fill: chartConfig.todo.color as string,
      },
      {
        name: chartConfig.inprogress.label as string,
        value: counts.inprogress,
        fill: chartConfig.inprogress.color as string,
      },
      {
        name: chartConfig.done.label as string,
        value: counts.done,
        fill: chartConfig.done.color as string,
      },
      {
        name: chartConfig.blocked.label as string,
        value: counts.blocked,
        fill: chartConfig.blocked.color as string,
      },
  ...
  const chartDescription = useMemo(() => {
    if (totalTasks === 0)
      return "No tasks found. Add some tasks to see your progress!";
  ...
  return (
    <Card className="shadow-lg h-full flex flex-col">
      <CardHeader>
        ...
        <CardDescription>
          {isLoading ? "Loading task data..." : chartDescription}
        </CardDescription>
      </CardHeader>
      <CardContent className="flex-1 pb-0 flex items-center justify-center">
        {isLoading ? (
          ...
          <p className="text-muted-foreground">
            No tasks with relevant statuses to display in chart.
          </p>
        ) : totalTasks === 0 ? (
          <p className="text-muted-foreground text-center px-4">
            Add tasks via the "My Tasks" page or by creating an AI schedule to
            see your progress distribution.
          </p>
        ) : (
          <ChartContainer
            config={chartConfig}
            className="mx-auto w-full aspect-square max-h-[260px] sm:max-h-[300px]"
          >
